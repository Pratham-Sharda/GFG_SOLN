...
class Solution{
public:
    void tree_created(node* &root0,vector<int> vec,int i){
        if(root0==NULL){
            return;
        }
        if(2*i+1<vec.size()){
            // node* lefter=new node(vec[2*i+1]);
            root0->left=newNode(vec[2*i+1]);}
        else{root0->left=NULL;}
        if(2*i +2<vec.size()){
            // node* righter=new node(vec[2*i+2]);
            root0->right=newNode(vec[2*i+2]);}
        else{root0->left=NULL;}
        tree_created(root0->left,vec,2*i+1);
        tree_created(root0->right,vec,2*i+2);    
        return;
        
    }
    void create_tree(node* root0, vector<int> &vec){
        //Your code goes here
        tree_created(root0, vec,0);
        return ;
    }
